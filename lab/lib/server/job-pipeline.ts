...
const HANDOFF_NOTES: Record<string, string> = {
  ...
  批准规格: "确认知识后由任务自动提交并批准不可变 SongSpec。",
...
export async function buildJobPipeline(job: JobRecord): Promise<JobStageView[]> {
  ...
  if (job.kind === "production_run") {
    ...
    const nextStage = stages.find((stage) => !reached.has(stage) && stage !== "人工评审") ?? null;
    let activeStage: string | null = null;
    if (state?.phase === "waiting_plan") activeStage = "知识确认";
    else if (state?.phase === "queued_approval" || state?.phase === "running_approval") activeStage = "批准规格";
    ...
    const waiting = state?.phase === "waiting_plan" || state?.phase === "waiting_generation";
    ...
    return stages.map((stage) => {
      ...
      if (job.status === "failed" && stage === nextStage) {
        return { stage, status: "failed" as const, note: job.error || "任务在这一步失败" };
      }
      if (stage === activeStage) {
        if (waiting) {
          const note = state?.phase === "waiting_plan"
            ? "等待核对学习目标、知识点、句尾答案和歌词。"
            : "等待确认后调用 MiniMax 生成隔离候选。";
      ...
      if (stage === "评分" && !reached.has("生成")) {
        return { stage, status: "pending" as const, note: "候选音频落盘后自动执行 8 维 ReportCard。" };
      }
  ...
  return stages.map((stage, index) => {
    ...
    // 本次任务真正负责的阶段：还没产物就是失败点或正在跑
    const ownedByThisJob = index <= lastIndex + 1 && !HANDOFF_NOTES[stage];
    if (ownedByThisJob && job.status === "failed") {
      return { stage, status: "failed" as const, note: job.error || "任务在这一步失败" };
    }
    if (ownedByThisJob && job.status === "running") {
      return { stage, status: "running" as const };
    }
